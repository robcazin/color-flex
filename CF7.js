...
const stripSWNumber = (colorName) => {
    ...
    return colorName.replace(/SC\d+\s*/, '').trim(); // Removes "SW" followed by digits and optional space
};
...
async function initializeApp() {
    ...
    try {
        ...
        populateLayerInputs(appState.curatedColors); // Calls handlePatternSelection here
...
const handlePatternSelection = (patternName) => {
    ...
    if (!appState.selectedCollection.tintWhite) {
...
const updatePreview = () => {
    ...
    const processPreview = async () => {
        const isTintWhitePath = appState.currentPattern?.tintWhite && appState.currentPattern?.baseComposite;
        const baseStepNeeded = appState.currentPattern?.baseComposite ? 1 : 0;
        const layerStepsNeeded = !isTintWhitePath ? (appState.currentPattern?.layers?.length || 0) : 0;
        const totalSteps = baseStepNeeded + layerStepsNeeded || 1;
        ...
        if (appState.currentPattern?.baseComposite) {
        ...
        if (!isTintWhitePath && appState.currentPattern?.layers?.length) {
            ...
            for (let index = 0; index < appState.currentPattern.layers.length; index++) {
                ...
                await new Promise((resolve) => {
                    processImage(layerUrl, (processedUrl) => {
                        ...
                        img.onload = () => {
                            console.log(`Layer ${index + 1} loaded: ${processedUrl}, Composite Mode: ${isShadow ? "multiply" : "source-over"}`);
        ...
        console.log("Updating preview DOM");
    ...
    // ... (Initial setup: dom checks, bgColor, UI constants, canvas, ctx) ...
    const updateRoomMockup = () => {
        ...
        const renderCanvas = () => {
            ...
            const existingProgress = dom.roomMockup.querySelector('.progress-container');
            dom.roomMockup.replaceChildren(existingProgress || []);
    
            try {
                const dataUrl = canvas.toDataURL("image/png");
                console.log(">>> Canvas Data URL length:", dataUrl.length);
                if (dataUrl.length < 100) {
                    console.warn(">>> Canvas Data URL seems very short - Canvas might be blank or nearly blank!");
                }
                const img = document.createElement("img");
                img.src = dataUrl;
        ...
        const processOverlay = async () => {
            ...
            console.log(">>> Current pattern:", appState.currentPattern);
    
            const isTintWhitePath = appState.currentPattern?.tintWhite && appState.currentPattern?.baseComposite;
            const baseStepNeeded = appState.currentPattern?.baseComposite ? 1 : 0;
            const layerStepsNeeded = !isTintWhitePath ? (appState.currentPattern?.layers?.length || 0) : 0;
            const shadowStepNeeded = appState.selectedCollection?.mockupShadow ? 1 : 0;
            const overlayStepNeeded = appState.selectedCollection?.mockup ? 1 : 0;
    
            const totalSteps = baseStepNeeded + layerStepsNeeded + shadowStepNeeded + overlayStepNeeded || 1;
            let completedSteps = 0;
            ...
            const progressContainer = document.createElement("div");
            progressContainer.className = "absolute inset-0 flex items-center justify-center z-10 progress-container";
            const progressBar = document.createElement("div");
            progressBar.className = "w-3/4 bg-blue-200 rounded-full h-4";
            const progressFill = document.createElement("div");
            progressFill.className = "bg-purple-600 h-4 rounded-full transition-all duration-300";
            ...
            dom.roomMockup.appendChild(progressContainer);
    
            const baseImage = new Image();
            let baseNeeded = appState.currentPattern?.baseComposite;
                    ...
                    baseImage.onload = () => {
                        ...
                        for (let x = -tileWidth; x < canvas.width + tileWidth; x += tileWidth) {
                            ...
                            for (let y = startY + yOffset; y < canvas.height + tileHeight; y += tileHeight) {
                                ...
                                console.log(`>>> Base tile at (${x}, ${y}), size: ${tileWidth}x${tileHeight}`);
                            }
                        }
                        console.log(">>> Base tiled with size:", tileWidth, "x", tileHeight, "offsetY:", offsetY);
                        updateProgress();
                        resolve();
                    };
                    baseImage.onerror = () => {
            ...
            if (mockupNeeded && mockupLoaded) {
                console.log(">>> Applying mockup overlay");
